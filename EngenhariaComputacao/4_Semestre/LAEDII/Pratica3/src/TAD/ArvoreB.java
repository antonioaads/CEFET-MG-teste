...
public class ArvoreB {
  ...
  private Item pesquisa (Item reg, Pagina ap) {
     pg_count++;
    if (ap == null){
        Variaveis.comparacoes+=1;   //Contagem das comparações realizadas para buscar um item
        return null;
      ...
      if (reg.compara (ap.r[i]) == 0){
        ...
        Variaveis.comparacoes+=2;   //Contagem das comparações realizadas para buscar um item
        return pesquisa (reg, ap.p[i]);
      }
      else{
        Variaveis.comparacoes+=3;   //Contagem das comparações realizadas para buscar um item
  ...
  private boolean reconstitui (Pagina apPag, Pagina apPai, int posPai) {
    ...
    if (posPai < apPai.n) { // Página a direita de apPag
      ...
      if (dispAux > 0) { //transfere de aux para apPag
        ...
        apPai.p[apPai.n--] = null; // Transfere a posse da memoria
        diminuiu = apPai.n < this.m;
      }      
    }
    else { //Página a esquerda
      Pagina aux = apPai.p[posPai-1];      
      int dispAux = (aux.n - this.m + 1)/2;
      ...
      apPag.n++;
      if (dispAux > 0) { // Transfere a posse da memoria
        for (int j = 0; j < dispAux - 1; j++) {
          this.insereNaPagina (apPag, aux.r[aux.n-j-1], aux.p[aux.n-j]);
          aux.p[aux.n-j] = null; // Transfere a posse da memoria
        }
        apPag.p[0] = aux.p[aux.n - dispAux + 1];
        aux.p[aux.n - dispAux + 1] = null; // Transfere a posse da memoria
        apPai.r[posPai-1] = aux.r[aux.n - dispAux];
        aux.n = aux.n - dispAux; diminuiu = false;
      }
      else { //intercala apPag em aux e libera apPag
        for (int j = 0; j < this.m; j++) {
          this.insereNaPagina (aux, apPag.r[j], apPag.p[j+1]);
          apPag.p[j+1] = null; // Transfere a posse da memoria
        }
        apPag = null; //Libera página
        apPai.p[apPai.n--] = null; // Transfere a posse da memoria
        diminuiu = apPai.n < this.m;
      }      
    }
  ...
  private Pagina retira (Item reg, Pagina ap, boolean[] diminuiu) {
    if (ap == null) {
      ...
      if (reg.compara (ap.r[ind]) == 0) { // encontrou
        if (ap.p[ind] == null) { //Página folha
          ...
          ap.p[ap.n+1] = null; // Transfere a posse da memoria
        }
        else { //Trocar com antecessor
  ...
  public ArvoreB (int m) {
    this.raiz = null; this.m = m; this.mm = 2*m; this.pg_count = 0;
  }
  ...
  public void retira (Item reg) {
    ...
    if (diminuiu[0] && (this.raiz.n == 0)) { //Diminui altura
